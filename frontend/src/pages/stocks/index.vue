...
const editingItem = ref<Stock | null>(null)
const form = ref({ name: '', symbol: '', shares: 0, buyPrice: 0, currentPrice: 0, broker: '', lastPriceUpdate: null as string | null })
...
const columns = [
  ...
  { key: 'pnl', label: '損益', align: 'right' as const },
  { key: 'broker', label: '券商' },
]
...
const stats = computed(() => {
  const totalValue = stocks.value.reduce((sum, s) => sum + s.currentPrice * s.shares, 0)
  const totalPnl = stocks.value.reduce((sum, s) => sum + (s.currentPrice - s.buyPrice) * s.shares, 0)
  return { totalValue, totalPnl, count: pagination.total.value }
})

function calcPnl(item: Stock) {
  return (item.currentPrice - item.buyPrice) * item.shares
}
...
function openCreate() {
  ...
  form.value = { name: '', symbol: '', shares: 0, buyPrice: 0, currentPrice: 0, broker: '', lastPriceUpdate: null }
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
      <Card>
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="text-xs text-text-secondary">總持有市值</p>
    ...
    <Card>
      <DataTable :columns="columns" :loading="loading" :items="stocks">
        ...
        <tr v-for="(item, idx) in stocks" :key="item.id" class="border-b border-border-default hover:bg-gray-100 dark:hover:bg-gray-700">
          ...
          <td class="py-3 px-4 text-sm text-right font-semibold" :class="calcPnl(item) >= 0 ? 'text-green-600' : 'text-red-600'">
            {{ formatMoney(calcPnl(item)) }}
          </td>
