...
const paymentMethodOptions = computed(() => props.paymentMethods
  .filter(paymentMethod => !isCreditCardPaymentMethod(paymentMethod))
...
const isHistoricalCreditCard = computed(() => Boolean(
  props.editing && isCreditCardPaymentMethod(props.editing.paymentMethod ?? selectedPaymentMethod.value),
))
const historicalPaymentMethodLabel = computed(() => isHistoricalCreditCard.value ? '信用卡' : (props.editing?.paymentMethod?.name ?? selectedPaymentMethod.value?.name ?? '信用卡'))
const referenceDataReady = computed(() => props.referenceDataReady)
const referenceError = computed(() => props.referenceDataError)
const fieldsDisabled = computed(() => props.submitting || props.submissionUncertain)
const canSubmit = computed(() => !props.submitting && !props.disabled && referenceDataReady.value && !referenceError.value && !props.submissionUncertain)
const actionLabel = computed(() => getTransactionActionLabel(form, Boolean(props.editing)))
...
watch(
  () => [form.type, form.categoryId, form.paymentMethodId],
  synchronizeDomainState,
  { flush: 'sync' },
)
  ...
  <form class="space-y-4" novalidate @submit.prevent="handleSubmit">
    ...
    <div>
      ...
      <Select
        ...
        :disabled="fieldsDisabled || isHistoricalCreditCard"
    ...
    <div>
      ...
      <Select
        v-if="!isHistoricalCreditCard"
        id="transaction-payment-method"
        :model-value="form.paymentMethodId ?? ''"
        :options="paymentMethodOptions"
        placeholder="選擇支付方式"
        :error="fieldError('paymentMethodId')"
        :disabled="fieldsDisabled || !referenceDataReady || Boolean(referenceError)"
      ...
      <Input
        v-else
        id="transaction-payment-method"
        :model-value="historicalPaymentMethodLabel"
        readonly
        aria-readonly="true"
        :disabled="fieldsDisabled"
      />
    ...
    <p v-if="isHistoricalCreditCard" class="rounded-lg bg-bg-raised px-3 py-2 text-xs text-text-secondary">
      信用卡付款時程請至「信用卡交易」調整。
    </p>
