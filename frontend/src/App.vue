...
watch(() => auth.authState.value, async (state) => {
  if (state === 'guest' && !route.meta?.public) {
...
<template>
  ...
  <MainLayout v-else-if="auth.authState.value === 'authenticated' && timeZone.isReady.value" />
