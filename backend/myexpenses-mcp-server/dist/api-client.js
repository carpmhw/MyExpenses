...
export class ApiClient {
    ...
    constructor(baseUrl, token, timeoutMs = 10_000, fetchImplementation = fetch) {
    ...
    /** 執行唯讀 GET 請求。 */
    async get(path) {
        const response = await this.request(path, { operation: 'read' });
    ...
    async request(path, options) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new ApiError(response.status, code, retryable ? '無法確認命令是否已提交，請使用原 requestId 重試' : safeMessage(response.status, code), retryable);
            }
            return {
                data: body,
            ...
            if (isAbortError(error)) {
                throw new ApiError(0, 'timeout', options.operation === 'write'
                    ? '無法確認命令是否已提交，請使用原 requestId 重試'
                    : '讀取 API 逾時，請稍後重試', true);
            }
            throw new ApiError(0, 'network_error', options.operation === 'write'
                ? '無法確認命令是否已提交，請使用原 requestId 重試'
                : '無法連線到記帳 API，請稍後重試', options.operation === 'write');
        }
        finally {
            clearTimeout(timer);
        }
...
function safeMessage(status, code) {
    if (code === 'result_unavailable')
        return '原命令已提交，但目前結果已不存在';
    if (code === 'conflict')
        return 'requestId 已被不同命令使用，請勿改寫原命令';
    if (code === 'forbidden')
        return 'API token 缺少執行此操作的 scope';
    if (code === 'unauthorized')
        return 'API token 無效或已失效';
    if (code === 'not_found')
        return '找不到指定的記帳資料';
    if (status >= 400 && status < 500)
        return '記帳 API 拒絕了這個請求，請檢查輸入資料';
