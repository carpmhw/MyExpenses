...
function toolResult(status, data, text, isError = false) {
    return {
...
/** 建立欄位追問結果，不執行任何寫入。 */
function needsInput(message, missingFields = [], fieldErrors = {}, extra = {}) {
...
function configurationError(code, message, extra = {}) {
...
function listItems(value, complete = false) {
    if (Array.isArray(value))
        return value;
    if (isRecord(value) && Array.isArray(value.items)) {
        if (complete && value.total !== value.items.length)
            throw new ToolInputError('invalid_api_response', 'API 參考資料集合不完整');
        return value.items;
    }
...
function transactionType(value) {
    ...
    if (normalized === 'income' || normalized === '收入')
        return 'Income';
    if (normalized === 'expense' || normalized === '支出')
        return 'Expense';
    return null;
}
/** 將分類 enum 輸入正規化為交易型別。 */
...
async function loadContext(client) {
    const value = await client.get('/api/agent/context');
...
async function loadCategories(client) {
    const items = listItems(await client.get('/api/categories?all=true'), true);
    requireApiSchema(arraySchema(categorySchema), items);
    return items;
}
/** 讀取所有付款方式參考資料。 */
async function loadPaymentMethods(client) {
    const items = listItems(await client.get('/api/payment-methods?all=true'), true);
    requireApiSchema(arraySchema(paymentSchema), items);
    return items;
}
/** 拒絕無效 API 資料，避免宣告成功或使用不完整參考資料。 */
function requireApiSchema(schema, value) {
    if (!matchesSchema(schema, value))
        throw new ToolInputError('invalid_api_response', 'API 回應格式無效，無法確認結果');
}
/** 讀取完整信用卡集合，避免單卡判斷受 API 分頁影響。 */
async function loadAllCreditCards(client) {
    let pageSize = 100;
    const cards = [];
    let page = 1;
    let total;
    do {
        const response = await client.get(queryPath('/api/credit-cards', { page, pageSize }));
        ...
        if (new Set(cards.map(card => card.id)).size !== cards.length)
            throw new ToolInputError('invalid_api_response', '信用卡分頁包含重複資料');
        page += 1;
    } while (cards.length < total && page <= 100);
    if (cards.length !== total)
        throw new ToolInputError('invalid_api_response', '信用卡分頁資料不完整');
    return cards;
}
/** 將分類轉成不含多餘欄位的候選資料。 */
function categoryCandidate(category) {
    return {
        id: category.id,
        name: category.name,
...
/** 將付款方式轉成不含多餘欄位的候選資料。 */
function paymentMethodCandidate(paymentMethod) {
    return {
        id: paymentMethod.id,
...
/** 將信用卡轉成可供使用者選擇的候選資料。 */
function creditCardCandidate(card) {
    return {
        id: card.id,
...
/** 判斷分類是否符合指定 selector。 */
function categoryMatches(category, selector) {
...
function resolveCategory(categories, args) {
    ...
    if (typeof categoryId === 'number') {
        ...
        if (matches.length === 0)
            return { value: null, result: needsInput('找不到指定分類', [], { categoryId: 'not_found' }) };
        ...
        if (matches.length === 0)
            return { value: null, result: needsInput('找不到指定分類', [], { category: 'not_found' }) };
    }
    if (matches.length > 1) {
        return {
            value: null,
...
function resolvePaymentMethod(paymentMethods, args) {
    ...
    if (typeof paymentMethodId === 'number') {
        ...
        if (matches.length === 0)
            return { value: null, result: needsInput('找不到指定付款方式', [], { paymentMethodId: 'not_found' }) };
        ...
        if (matches.length === 0)
            return { value: null, result: needsInput('找不到指定付款方式', [], { paymentMethod: 'not_found' }) };
    }
    if (matches.length > 1) {
        return {
            value: null,
...
function resolveCreditCard(cards, args) {
    ...
    if (cardId !== undefined) {
        ...
        if (matches.length === 0)
            return { value: null, result: needsInput('找不到指定信用卡', [], { cardId: 'not_found' }) };
    ...
    if (selector === undefined) {
        ...
        if (cards.length === 0)
            return { value: null, result: configurationError('credit_card_not_configured', '目前沒有可用信用卡') };
        return {
            value: null,
    ...
    if (typeof selector !== 'string')
        return { value: null, result: needsInput('信用卡 selector 必須是文字或 cardId', [], { card: 'invalid' }) };
    const matches = cards.filter(card => creditCardMatches(card, selector));
    if (matches.length === 0)
        return { value: null, result: needsInput('找不到指定信用卡', [], { card: 'not_found' }) };
    if (matches.length > 1) {
        return {
            value: null,
...
async function resolveDate(args, client) {
...
function intentValue(value) {
    if (value === 'ordinary' || value === 'credit_card_purchase' || value === 'credit_card_repayment')
        return value;
    return null;
}
/** 準備普通收入／支出的固定 canonical 命令。 */
async function prepareOrdinary(args, client, uuid) {
    const amountValue = args.amount;
    if (amountValue === undefined || amountValue === null)
        return needsInput('請提供金額', ['amount']);
    let amount;
    try {
        amount = positiveAmount(amountValue);
    }
    catch (error) {
        return needsInput(error.message, [], { amount: 'invalid' });
    }
    const description = typeof args.description === 'string' ? args.description.trim() : '';
    if (!description)
        return needsInput('請提供交易描述', ['description']);
    ...
    if (!category) {
        if (explicitType === 'Income')
            return needsInput('收入交易需要明確分類', ['category']);
        ...
        if (!category)
            return configurationError('default_category_missing', '找不到預設分類 other-expense');
        defaults.push('category=other-expense');
    }
    const resolvedCategoryType = categoryType(category.type);
    if (!resolvedCategoryType)
        return configurationError('invalid_category_type', '分類參考資料的 type 無效');
    ...
    if (!paymentMethod) {
        ...
        if (!paymentMethod)
            return configurationError('default_payment_method_missing', '找不到預設付款方式 cash');
    ...
    if (dateResult.defaulted)
        defaults.push(`date=${dateResult.date}`);
...
async function prepareCreditCardRepayment(args, client, uuid) {
    const amountValue = args.amount;
    if (amountValue === undefined || amountValue === null)
        return needsInput('請提供卡費繳款金額', ['amount']);
    let amount;
    try {
        amount = positiveAmount(amountValue);
    }
    ...
    if (!category) {
        ...
        if (!category)
            return configurationError('repayment_category_missing', '找不到卡費繳款需要的 living 分類');
    ...
    if (categoryType(category.type) !== 'Expense')
        return configurationError('invalid_category_type', 'living 分類必須是支出型別');
    const paymentResult = resolvePaymentMethod(paymentMethods, args);
    if (paymentResult.result)
        return paymentResult.result;
    let paymentMethod = paymentResult.value;
    ...
    if (!originalDescription)
        defaults.push('description=信用卡帳單');
    if (dateResult.defaulted)
        defaults.push(`date=${dateResult.date}`);
    const requestId = uuid();
    const command = {
...
async function prepareCreditCardPurchase(args, client, uuid) {
    const amountValue = args.amount ?? args.totalAmount;
    if (amountValue === undefined || amountValue === null)
        return needsInput('請提供信用卡消費總額', ['amount']);
    let amount;
    try {
    ...
    const description = typeof args.description === 'string' ? args.description.trim() : '';
    if (!description)
        return needsInput('請提供信用卡消費描述', ['description']);
    const dateResult = await resolveDate(args, client);
    if (!isResolvedDate(dateResult))
        return dateResult;
    ...
    if (!cardResult.value)
        return configurationError('credit_card_not_configured', '目前沒有可用信用卡');
    ...
    if (args.periods === undefined || args.periods === null) {
        if (requestedInstallment === true)
            return needsInput('已指定分期，請提供期數', ['periods']);
        periods = 1;
    ...
    if (dateResult.defaulted)
        defaults.push(`purchaseDate=${dateResult.date}`);
    if (args.card === undefined && args.cardId === undefined)
        defaults.push(`cardId=${cardResult.value.id}`);
    const requestId = uuid();
    const command = {
        requestId,
...
async function prepareBookkeepingEntry(args, client, uuid) {
    const intent = intentValue(args.intent);
    if (!intent)
        return needsInput('請指定 ordinary、credit_card_purchase 或 credit_card_repayment intent', ['intent']);
    ...
    for (const field of incompatible)
        if (args[field] !== undefined)
            fieldErrors[field] = 'unsupported_for_intent';
    if (Object.keys(fieldErrors).length)
        return needsInput('指定欄位不適用於此記帳意圖', [], fieldErrors);
    if (intent === 'credit_card_purchase') {
        if (args.amount !== undefined && args.totalAmount !== undefined && args.amount !== args.totalAmount)
            return needsInput('amount 與 totalAmount 不一致，請確認總額', [], { totalAmount: 'conflict' });
        if (args.perPeriodAmount !== undefined && args.totalAmount === undefined)
            return needsInput('只有每期金額不足以確認總額，請提供 totalAmount', ['totalAmount']);
    }
    if (intent === 'ordinary')
        return prepareOrdinary(args, client, uuid);
    if (intent === 'credit_card_repayment')
        return prepareCreditCardRepayment(args, client, uuid);
    return prepareCreditCardPurchase(args, client, uuid);
}
/** 產生缺少準備 envelope 時的安全提示，不呼叫寫入 API。 */
function needsPreparation(targetTool, missingFields) {
    return errorResult('needs_preparation', 'needs_preparation', `請先呼叫 prepare_bookkeeping_entry 取得固定日期、參考資料與 requestId：${missingFields.join(', ')}`, { targetTool, missingFields, guidance: '準備完成後，原樣傳入 arguments 與 requestId。' });
}
/** 取得普通交易 canonical response 中適合回傳的欄位。 */
function transactionData(value) {
    requireApiSchema(transactionSchema, value);
    if (!isRecord(value))
        throw new ToolInputError('invalid_api_response', '普通交易 API 回應格式無效');
    return {
        id: value.id,
        type: value.type,
        amount: value.amount,
...
async function createTransaction(args, client) {
    ...
    if (args.type === undefined)
        missing.push('type');
    if (missing.length > 0)
        return needsPreparation('create_transaction', missing);
    if (!validUuid(requestId))
        return errorResult('error', 'invalid_request_id', 'requestId 必須是 UUID');
    if (!validDate(args.date))
        return errorResult('error', 'invalid_date', 'date 必須是有效的 YYYY-MM-DD');
    let amount;
    try {
        amount = positiveAmount(args.amount);
    }
    catch (error) {
        return errorResult('error', 'invalid_amount', error.message, { requestId });
    }
    const type = transactionType(args.type);
    if (!type)
        return errorResult('error', 'invalid_type', 'type 必須是 Income 或 Expense', { requestId });
    let categoryId;
    let paymentMethodId;
    try {
        ...
        return errorResult('error', 'invalid_reference_id', error.message, { requestId });
    }
    ...
    const response = await client.post('/api/transactions', body, requestId);
...
function canonicalReferenceLabel(reference, id) {
    if (isRecord(reference) && typeof reference.name === 'string' && reference.name.trim())
        return reference.name.trim();
    return typeof id === 'number' ? `ID ${id}` : '未提供（ID 不可用）';
}
/** 取得信用卡交易 canonical response 中適合回傳的欄位。 */
function creditCardTransactionData(value) {
    requireApiSchema(creditSchema, value);
    if (!isRecord(value))
        throw new ToolInputError('invalid_api_response', '信用卡交易 API 回應格式無效');
    return {
        id: value.id,
...
async function createCreditCardTransaction(args, client) {
    const requestId = args.requestId;
    const missing = [];
    ...
    if (missing.length > 0)
        return needsPreparation('create_credit_card_transaction', missing);
    if (!validUuid(requestId))
        return errorResult('error', 'invalid_request_id', 'requestId 必須是 UUID');
    if (!validDate(args.purchaseDate))
        return errorResult('error', 'invalid_date', 'purchaseDate 必須是有效的 YYYY-MM-DD', { requestId });
    let cardId;
    let periods;
    let totalAmount;
    try {
        ...
        return errorResult('error', 'invalid_input', error.message, { requestId });
    }
    if (periods > 60)
        return errorResult('error', 'periods_out_of_range', 'periods 必須介於 1 與 60', { requestId });
    if (typeof args.description !== 'string' || args.description.trim() === '') {
        return errorResult('error', 'invalid_description', 'description 不可為空', { requestId });
    }
    const body = {
    ...
    const response = await client.post('/api/installments', body, requestId);
...
async function listCategories(args, client) {
...
async function getRecentTransactions(args, client) {
    const limit = args.limit === undefined ? 5 : args.limit;
    if (typeof limit !== 'number' || !Number.isInteger(limit) || limit < 1 || limit > 100) {
        return errorResult('error', 'invalid_limit', 'limit 必須是 1 到 100 的整數');
    }
    const data = await client.get(queryPath('/api/transactions', { limit }));
    const items = Array.isArray(data) ? data.map(transactionData) : listItems(data).map(transactionData);
    return toolResult('ok', { items }, `取得最近 ${items.length} 筆普通交易`);
}
/** 取得完整信用卡候選清單與分頁資訊。 */
async function listCreditCards(args, client) {
    ...
    if (pageSize > 100)
        return errorResult('error', 'invalid_page_size', 'pageSize 不可超過 100');
    const data = await client.get(queryPath('/api/credit-cards', { page, pageSize }));
    if (!isRecord(data))
        throw new ToolInputError('invalid_api_response', '信用卡列表回應格式無效');
    requireApiSchema(pageSchema(cardSchema), data);
    const items = listItems(data).map(creditCardCandidate);
    if (data.page !== page || items.length !== Math.min(data.pageSize, Math.max(0, data.total - (page - 1) * data.pageSize))
...
async function undoTransaction(args, client) {
    let id;
    try {
        id = positiveInteger(args.id, 'id');
    }
    catch (error) {
        return errorResult('error', 'invalid_id', error.message);
    }
    const response = await client.post(`/api/transactions/${id}/undo`, {});
    const transaction = transactionData(response.data);
...
async function financialSummary(client) {
    const summary = await client.get('/api/reports/monthly-summary');
    return toolResult('ok', { summary, basis: 'ordinary_financial_summary' }, '已取得目前月份普通收支摘要');
}
/** 讀取普通交易明細。 */
async function getTransaction(args, client) {
    let id;
    try {
        id = positiveInteger(args.id, 'id');
    }
    ...
    const transaction = transactionData(await client.get(`/api/transactions/${id}`));
    return toolResult('ok', { sourceType: 'ordinary', sourceId: id, transaction }, `取得普通交易 #${id}`);
}
/** 讀取獨立信用卡交易與付款時程。 */
async function getCreditCardTransaction(args, client) {
    let id;
    try {
        id = positiveInteger(args.sourceId ?? args.id, 'sourceId');
    }
    catch (error) {
        return errorResult('error', 'invalid_source_id', error.message);
    }
    const transaction = creditCardTransactionData(await client.get(`/api/installments/${id}`));
    return toolResult('ok', { sourceType: 'credit_card', sourceId: id, transaction }, `取得信用卡交易 #${id}`);
}
/** 執行普通交易原始帳目查詢並保留完整 filtered summary。 */
async function searchTransactions(args, client) {
    const values = {};
    for (const field of ['startDate', 'endDate']) {
        if (args[field] !== undefined && !validDate(args[field]))
            return errorResult('error', 'invalid_date', `${field} 必須是有效的 YYYY-MM-DD`);
        values[field] = args[field];
    }
    if (args.categoryId !== undefined) {
        try {
            values.categoryId = positiveInteger(args.categoryId, 'categoryId');
        }
        catch (error) {
            return errorResult('error', 'invalid_category_id', error.message);
        }
    }
    if (args.category !== undefined || args.categoryCode !== undefined) {
        ...
        if (!categoryResult.value)
            return errorResult('error', 'invalid_category_id', '找不到指定分類');
    ...
    if (searchValue !== undefined && typeof searchValue !== 'string')
        return errorResult('error', 'invalid_search', 'search 必須是文字');
    if (args.type !== undefined) {
        const type = transactionType(args.type);
        if (!type)
            return errorResult('error', 'invalid_type', 'type 必須是 income 或 expense');
    ...
    if (args.repaymentOnly !== undefined && typeof args.repaymentOnly !== 'boolean')
        return errorResult('error', 'invalid_repayment_only', 'repaymentOnly 必須是 boolean');
    for (const field of ['page', 'pageSize', 'repaymentOnly'])
        values[field] = args[field];
    values.search = searchValue;
    if (values.page !== undefined) {
        try {
            positiveInteger(values.page, 'page');
        }
        catch (error) {
            return errorResult('error', 'invalid_page', error.message);
        }
    }
    if (values.pageSize !== undefined) {
        try {
            ...
            return errorResult('error', 'invalid_page_size', error.message);
        }
    }
    const data = await client.get(queryPath('/api/transactions', values));
    if (!isRecord(data))
        throw new ToolInputError('invalid_api_response', '交易查詢回應格式無效');
    return toolResult('ok', { ...data }, transactionListText(data));
}
/** 執行跨來源 consumption 查詢，保留期間與 coverage 說明。 */
async function searchConsumption(args, client) {
    for (const field of ['startDate', 'endDate']) {
        if (!validDate(args[field]))
            return errorResult('error', 'invalid_date', `${field} 必須是有效的 YYYY-MM-DD`);
    }
    const values = {
    ...
    if (args.page !== undefined) {
        try {
            values.page = positiveInteger(args.page, 'page');
        }
        catch (error) {
            return errorResult('error', 'invalid_page', error.message);
        }
    ...
    const data = await client.get(queryPath('/api/consumption', values));
    if (!isRecord(data))
        throw new ToolInputError('invalid_api_response', 'consumption 查詢回應格式無效');
...
async function handleTool(name, args, client, uuid) {
    switch (name) {
        ...
        case 'prepare_bookkeeping_entry':
            return prepareBookkeepingEntry(args, client, uuid);
...
function mapFailure(name, args, error) {
    ...
    if (error instanceof ToolInputError)
        return errorResult('error', error.code, error.message, requestId);
    return errorResult('error', 'internal_error', '工具執行失敗，未建立可確認的結果', requestId);
}
/** 建立 MCP server、註冊工具列表與安全的 tools/call handler。 */
export function createServer(client, uuid = randomUUID) {
    const server = new Server({ name: 'myexpenses-mcp-server', version: '2.0.0' }, { capabilities: { tools: {} } });
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            if (definition)
                requireApiSchema(definition.outputSchema, result.structuredContent);
            return result;
        }
        catch (error) {
