...
const server = new Server({ name: 'myexpenses-mcp-server', version: '1.0.0' }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        case 'create_transaction': {
            const { amount, description, category, date, paymentMethod, notes } = args;
        ...
        case 'list_categories': {
            ...
            const filtered = type ? items.filter((c) => c.type.toLowerCase() === type.toLowerCase()) : items;
            return {
                content: [{ type: 'text', text: JSON.stringify(filtered.map((c) => ({
        ...
        case 'list_payment_methods': {
            ...
            const items = Array.isArray(result) ? result : result.items || [];
            return {
                content: [{ type: 'text', text: JSON.stringify(items.map((p) => ({
