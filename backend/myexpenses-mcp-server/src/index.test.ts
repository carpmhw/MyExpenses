...
import { ApiClient, type FetchImplementation } from './api-client.js';
...
function mockFetch(handler: RouteHandler, requests: Array<{ url: URL; init: RequestInit }>): FetchImplementation {
...
async function callTool(
  ...
  uuid: string | null = '11111111-1111-4111-8111-111111111111',
): Promise<{ result: Record<string, unknown>; requests: Array<{ url: URL; init: RequestInit }> }> {
  ...
  await client.close();
  await server.close();
  return { result: result as unknown as Record<string, unknown>, requests };
}
...
test('historical 72-period credit details and edited canonical replays retain all payments', async () => {
  ...
  for (const replayed of [false, true]) {
    const { result, requests } = await callTool((url, init) => {
      ...
      if (replayed) assert.equal(JSON.parse(String(init.body)).periods, 12);
    ...
    const transaction = (replayed ? data.creditCardTransaction : data.transaction) as Record<string, unknown>;
...
function references(url: URL): Response {
  if (url.pathname === '/api/agent/context') return Response.json({ currentDate: '2026-09-05', timeZoneId: 'Asia/Taipei' });
  if (url.pathname === '/api/categories') return Response.json([{ id: 7, name: '飲食', type: 'Expense', systemCode: 'food' }, { id: 8, name: '其他', type: 'Expense', systemCode: 'other-expense' }]);
  if (url.pathname === '/api/payment-methods') return Response.json([{ id: 3, name: '現金', systemCode: 'cash' }]);
...
test('independent preparations generate distinct real UUIDs without financial writes', async () => {
  ...
  const { requestId: firstId, ...firstPayload } = a.arguments as Record<string, unknown>;
  const { requestId: secondId, ...secondPayload } = b.arguments as Record<string, unknown>;
...
test('missing default category, cash, living or cards produces configuration errors', async () => {
  for (const [path, intent, code] of [
    ...
    const { result, requests } = await callTool(url => url.pathname === path
      ? Response.json(path === '/api/credit-cards' ? { items: [], total: 0, page: 1, pageSize: 100 } : []) : references(url),
    'prepare_bookkeeping_entry', { intent, amount: 150, description: '午餐' });
...
test('missing bookkeeping fields and duplicate card names request clarification', async () => {
  ...
  const { result, requests } = await callTool(url => url.pathname === '/api/credit-cards'
    ? Response.json({ items: [{ id: 1, bankName: '同名銀行', lastFourDigits: '1111' }, { id: 2, bankName: '同名銀行', lastFourDigits: '2222' }], total: 2, page: 1, pageSize: 100 }) : references(url),
  'prepare_bookkeeping_entry', { intent: 'credit_card_purchase', amount: 150, description: '午餐', card: '同名銀行' });
  assert.equal(structured(result).status, 'needs_input');
  assert.deepEqual(structured(result).fieldErrors, { card: 'ambiguous' });
  assert.equal((structured(result).candidates as unknown[]).length, 2);
  assert.ok(requests.every(request => request.init.method === 'GET'));
});

// 驗證錯誤碼與 HTTP 狀態一致，annotations 不會繞過後端拒絕或洩漏內容。
test('401, 403, 409 and 410 return explicit safe codes for both write tools', async () => {
  for (const [status, code] of [[401, 'unauthorized'], [403, 'forbidden'], [409, 'conflict'], [410, 'result_unavailable']] as const) {
    for (const [name, command] of [['create_transaction', ordinaryCommand], ['create_credit_card_transaction', creditCommand]] as const) {
...
test('real stdio process restart across midnight preserves ordinary and credit envelopes', async () => {
  ...
  const writes: Array<{ path: string; key: string | undefined; body: string }> = [];
  const reads: string[] = [];
  ...
  try {
    for (const restarted of [false, true]) {
      if (restarted) currentDate = '2026-09-06';
      ...
      try {
        ...
        if (!restarted) {
          for (const args of [{ intent: 'ordinary', amount: 150, description: '午餐' },
            ...
            const prepared = (await client.callTool({ name: 'prepare_bookkeeping_entry', arguments: args })).structuredContent as Record<string, unknown>;
            assert.equal(prepared.status, 'ready');
            envelopes.push({ name: prepared.targetTool as string, arguments: prepared.arguments as Record<string, unknown> });
          }
        } else {
          const context = (await client.callTool({ name: 'get_bookkeeping_context' })).structuredContent as Record<string, unknown>;
          assert.equal((context.context as Record<string, unknown>).currentDate, '2026-09-06');
        }
        const readCount = reads.length;
        for (const envelope of envelopes) {
          const data = (await client.callTool(envelope)).structuredContent as Record<string, unknown>;
...
test('malformed read payloads fail closed', async () => {
  for (const name of ['search_consumption', 'search_transactions', 'get_financial_summary', 'get_transaction', 'get_credit_card_transaction', 'list_credit_cards', 'list_categories', 'list_payment_methods']) {
    for (const payload of [{}, { items: [{}], total: 1 }, { items: [], total: 0, summary: {} }]) {
      if (['list_categories', 'list_payment_methods'].includes(name) && payload.total === 0) continue;
      const { result } = await callTool(() => Response.json(payload), name,
        name === 'get_transaction' ? { id: 1 } : name === 'get_credit_card_transaction' ? { sourceId: 1 } : name.startsWith('search_') ? { startDate: '2026-09-01', endDate: '2026-09-05' } : {});
...
test('reference names and codes resolve the same identity', async () => {
  ...
  assert.equal((structured(result).arguments as Record<string, unknown>).categoryId, 7);
});
...
test('card pagination rejects missing totals, stalled pages, changing totals and duplicates', async () => {
  for (const mode of ['missing', 'stalled', 'changing', 'duplicate', 'cap']) {
    const { result, requests } = await callTool(url => {
      if (url.pathname !== '/api/credit-cards') return references(url);
  ...
  for (const name of ['create_transaction', 'create_credit_card_transaction']) {
    ...
    const { result, requests } = await callTool((_url, init) => {
      ...
      return Response.json(name === 'create_transaction'
        ...
        : { ...creditCommand, id: 42, cardId: 99, purchaseDate: '2026-09-06', totalAmount: 999, perPeriod: 83.25, payments: [] },
  ...
  for (const mode of ['partial', 'duplicate_category', 'duplicate_payment', 'income_repayment']) {
    const { result } = await callTool(url => {
      if (url.pathname === '/api/categories') {
        ...
        if (mode === 'partial') return Response.json({ items: [category], total: 2 });
        return Response.json(mode === 'duplicate_category' ? [category, { ...category, id: 9 }] : [category]);
      }
      if (mode === 'duplicate_payment' && url.pathname === '/api/payment-methods') return Response.json([{ id: 1, name: '現金', systemCode: 'cash' }, { id: 2, name: '現金2', systemCode: 'cash' }]);
...
test('write errors preserve safe status and never leak backend bodies', async () => {
  for (const status of [401, 403, 404, 409, 410, 422, 500, 200]) {
    ...
    assert.equal(requests.length, 1);
    if (status === 410) assert.equal(structured(result).code, 'result_unavailable');
  }
});
...
test('consumption preserves complete summary and coverage and rejects malformed nested fields', async () => {
  ...
  for (const extra of [{}, { summary: { ...payload.summary, totalAmount: '50000' } }, { items: [{ ...payload.items[0], sourceId: null }] }, { coverage: {} }, { period: { startDate: '2026-02-30', endDate: '2026-09-05' } }]) {
    ...
    if (!Object.keys(extra).length) assert.deepEqual(structured(result).summary, payload.summary);
  }
});
...
test('real stdio tools/list and tools/call validate output and reject invalid input', async () => {
  ...
  try {
    ...
    assert.equal((context.structuredContent as Record<string, unknown>).status, 'ok');
    ...
    assert.equal((missing.structuredContent as Record<string, unknown>).status, 'needs_preparation');
...
test('preparation rejects a context without a usable timezone', async () => {
  const { result } = await callTool(url => url.pathname === '/api/agent/context'
    ? Response.json({ currentDate: '2026-09-05', timeZoneId: ' ' }) : references(url),
...
test('bounded request timeout aborts the write and preserves its envelope', async () => {
  const { result, requests } = await callTool((_url, init) => new Promise<Response>((_resolve, reject) => {
...
test('prepare_bookkeeping_entry fixes backend date and reference IDs without writing', async () => {
  ...
  const command = data.arguments as Record<string, unknown>;
...
test('create_transaction refuses unprepared input and uses requestId for write', async () => {
  ...
  const executed = await callTool((url, init) => {
    assert.equal(url.pathname, '/api/transactions');
    assert.equal(init.headers && (init.headers as Record<string, string>)['Idempotency-Key'], '11111111-1111-4111-8111-111111111111');
  ...
  assert.equal((structured(executed.result).transaction as Record<string, unknown>).id, 9);
});
...
test('credit purchase preparation asks for a card when several cards exist', async () => {
  ...
  assert.equal((data.candidates as unknown[]).length, 2);
});
...
test('repayment preparation normalizes living category and avoids duplicate marker', async () => {
  const { result } = await callTool((url) => {
    if (url.pathname === '/api/agent/context') return Response.json({ currentDate: '2026-09-05', timeZoneId: 'Asia/Taipei' });
    if (url.pathname === '/api/categories') return Response.json([{ id: 8, name: '生活改名', type: 'Expense', systemCode: 'living' }]);
    if (url.pathname === '/api/payment-methods') return Response.json([{ id: 3, name: '現金', systemCode: 'cash' }]);
    return Response.json({}, { status: 500 });
  }, 'prepare_bookkeeping_entry', {
